...
const ClassCardTutor: React.FC<ClassCardProps> = ({ class: { data } }) => {
  ...
  return (
    <>
      {data && data != null ? (
        <Link
          href={`/tutor/${data.subjectId}`}
          className="flex w-80 cursor-pointer flex-col space-y-0 bg-transparent transition-all hover:scale-105 sm:w-60"
        >
          <Image
          ...
          {/* Top Section */}
          <div className="relative flex h-60 w-80 items-end justify-center sm:w-60">
            <Image
              src={BGimage}
          ...
          {/* Bottom Section */}
          <div className="flex min-h-24 w-full flex-col justify-between gap-3 bg-dark-brown p-4 text-white">
